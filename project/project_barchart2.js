...
class BarChart2 {
  ...
  render() {
    ...
    .on('mouseover', (e,d) => {
        d3.select('#tooltip')
            .style('opacity', 1)
            .html(`<div class="tooltip-label">Prefecture:${d.prefecture}</div>Forest_ratio: ${d.forest_ratio}<br>Population_density(人/ha): ${d.population_density}<br>Population(人): ${d.population}`);
    })
    .on('mousemove', (e) => {
        const padding = 10;
        d3.select('#tooltip')
